...
export default function Equipe({ equipe, setEquipe, pointages, setPointages, chantiers, couleur }) {
  ...
  const startChrono = () => { if (!chrono.employeId) return alert('SÃ©lectionnez un employÃ©'); setChrono(p => ({ ...p, running: true, start: Date.now() })); };
  ...
  const validerSemaine = () => {
    if (!confirm('Valider et verrouiller les heures de la semaine ? Cette action est irrÃ©versible.')) return;
  ...
  const exportCSV = () => {
    const rows = [['Date', 'EmployÃ©', 'Chantier', 'Heures', 'Statut', 'Note']];
    weekPointages.forEach(p => { const emp = equipe.find(e => e.id === p.employeId); const ch = chantiers.find(c => c.id === p.chantierId); rows.push([p.date, `${emp?.nom || ''} ${emp?.prenom || ''}`, ch?.nom || '-', p.heures, p.verrouille ? 'VerrouillÃ©' : p.approuve ? 'ValidÃ©' : 'En attente', p.note || '']); });
  ...
  if (showAdd) return (
    <div className="space-y-6">
      <div className="flex items-center gap-4"><button onClick={() => { setShowAdd(false); setEditId(null); setForm({ nom: '', prenom: '', telephone: '', tauxHoraire: '', coutHoraireCharge: '' }); }} className="p-2 hover:bg-slate-100 rounded-xl">â†</button><h1 className="text-2xl font-bold">{editId ? 'Modifier' : 'Nouvel'} employÃ©</h1></div>
      <div className="bg-white rounded-2xl border p-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Taux facturation (â‚¬/h)</label><input type="number" className="w-full px-4 py-2.5 border rounded-xl" value={form.tauxHoraire} onChange={e => setForm(p => ({...p, tauxHoraire: e.target.value}))} placeholder="45" /></div>
          <div className="col-span-2">
            <label className="block text-sm font-medium mb-1">CoÃ»t horaire chargÃ© (â‚¬/h) *</label>
            ...
            <p className="text-xs text-slate-500 mt-1">Salaire brut + charges patronales (~45%). UtilisÃ© pour calculer la rentabilitÃ© rÃ©elle des chantiers.</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4"><h1 className="text-2xl font-bold">Ã‰quipe & Heures</h1><button onClick={() => setShowAdd(true)} className="px-4 py-2 text-white rounded-xl" style={{background: couleur}}>+ EmployÃ©</button></div>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-5 text-white">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right"><p className="text-sm opacity-80">ValidÃ©es</p><p className="text-2xl font-bold">{approvedWeekHours.toFixed(1)}h</p></div>
        </div>
      ...
      <div className="flex gap-2 border-b pb-2 overflow-x-auto">
        {[['pointage', 'â±ï¸ Pointage'], ['equipe', 'ðŸ‘· Ã‰quipe'], ['validation', `âœ… Validation ${pointagesEnAttente.length > 0 ? `(${pointagesEnAttente.length})` : ''}`], ['historique', 'ðŸ“‹ Historique']].map(([k, v]) => (
      ...
      {tab === 'pointage' && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border p-6">
            <h3 className="font-semibold mb-4">â±ï¸ ChronomÃ¨tre</h3>
            <div className="text-center mb-6"><p className="text-5xl font-mono font-bold" style={{color: chrono.running ? couleur : '#64748b'}}>{formatTime(elapsed)}</p>{chrono.running && <p className="text-sm text-orange-500 mt-2">ðŸ”´ En cours</p>}</div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <select className="px-4 py-2.5 border rounded-xl" value={chrono.employeId} onChange={e => setChrono(p => ({...p, employeId: e.target.value}))} disabled={chrono.running}><option value="">EmployÃ© *</option>{equipe.map(e => <option key={e.id} value={e.id}>{e.nom} {e.prenom}</option>)}</select>
            ...
            <div className="flex justify-center gap-4">
              {!chrono.running ? <button onClick={startChrono} className="px-8 py-3 text-white rounded-xl text-lg" style={{background: couleur}}>â–¶ï¸ DÃ©marrer</button> : <button onClick={() => { const note = prompt('Note de fin (optionnel):'); stopChrono(note || ''); }} className="px-8 py-3 bg-red-500 text-white rounded-xl text-lg">â¹ï¸ ArrÃªter</button>}
            </div>
          </div>
          <div className="bg-white rounded-2xl border p-6">
            <h3 className="font-semibold mb-4">âœï¸ Saisie manuelle</h3>
            <div className="flex gap-3 flex-wrap">
              <select className="flex-1 min-w-[140px] px-4 py-2.5 border rounded-xl" value={pForm.employeId} onChange={e => setPForm(p => ({...p, employeId: e.target.value}))}><option value="">EmployÃ© *</option>{equipe.map(e => <option key={e.id} value={e.id}>{e.nom}</option>)}</select>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              {pointagesEnAttente.length > 0 && <button onClick={approuverTout} className="px-4 py-2 bg-emerald-500 text-white rounded-xl text-sm">âœ… Tout valider</button>}
              <button onClick={validerSemaine} className="px-4 py-2 bg-blue-500 text-white rounded-xl text-sm">ðŸ”’ Verrouiller semaine</button>
            </div>
          </div>
          {pointagesEnAttente.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">âœ…</p><p className="text-slate-500">Tous validÃ©s</p></div> : (
              ...
              {pointagesEnAttente.map(p => {
                ...
                return (
                  <div key={p.id} className="flex items-center px-5 py-4 border-b gap-4 flex-wrap">
                    <span className={`text-2xl ${p.manuel ? 'ðŸ”µ' : 'ðŸŸ '}`}></span>
                    <div className="flex-1 min-w-[150px]"><p className="font-medium">{emp?.nom} {emp?.prenom}</p><p className="text-sm text-slate-500">{ch?.nom || 'Sans chantier'} â€¢ {new Date(p.date).toLocaleDateString('fr-FR')}</p>{p.note && <p className="text-xs text-blue-600 mt-1">ðŸ’¬ {p.note}</p>}</div>
                    ...
                    <button onClick={() => approuverPointage(p.id)} className="px-4 py-2 bg-emerald-500 text-white rounded-xl text-sm">âœ…</button>
                    <button onClick={() => rejeterPointage(p.id)} className="px-4 py-2 bg-red-100 text-red-600 rounded-xl text-sm">ðŸ—‘ï¸</button>
                  </div>
                );
              })}
            </div>
      ...
      {tab === 'equipe' && (
        equipe.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">ðŸ‘·</p><p className="text-slate-500">Ajoutez votre Ã©quipe</p></div> : (
            ...
            {equipe.map(e => (
              <div key={e.id} className="bg-white rounded-2xl border p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <button onClick={() => startEdit(e)} className="text-slate-400 hover:text-slate-600">âœï¸</button>
                  <button onClick={() => deleteEmploye(e.id)} className="text-red-400 hover:text-red-600">ðŸ—‘ï¸</button>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                  <div className="bg-slate-50 p-3 rounded-xl"><p className="text-xs text-slate-500">Taux facturÃ©</p><p className="font-bold">{e.tauxHoraire || 45}â‚¬/h</p></div>
                  <div className="bg-red-50 p-3 rounded-xl"><p className="text-xs text-slate-500">CoÃ»t chargÃ©</p><p className="font-bold text-red-600">{e.coutHoraireCharge || 28}â‚¬/h</p></div>
                </div>
                <div className="flex justify-between items-center pt-4 border-t"><div><p className="text-2xl font-bold" style={{color: couleur}}>{getHeuresMois(e.id).toFixed(1)}h</p><p className="text-xs text-slate-500">ce mois (validÃ©)</p></div></div>
              </div>
            ))}
          </div>
        )
      ...
      {tab === 'historique' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center"><p className="text-sm text-slate-500">Semaine du {weekStart.toLocaleDateString('fr-FR')}</p><button onClick={exportCSV} className="px-4 py-2 rounded-xl text-sm" style={{background: `${couleur}20`, color: couleur}}>ðŸ“¥ Exporter CSV</button></div>
          <div className="bg-white rounded-2xl border overflow-hidden">
            {weekPointages.length === 0 ? <p className="p-8 text-center text-slate-500">Aucun pointage</p> : weekPointages.sort((a, b) => new Date(b.date) - new Date(a.date)).map(p => {
              ...
              return (
                <div key={p.id} className="flex items-center px-5 py-3 border-b gap-4">
                  <span className={`text-lg ${p.verrouille ? 'ðŸ”’' : p.approuve ? 'âœ…' : 'â³'}`}></span>
