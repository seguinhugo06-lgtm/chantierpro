...
export function generateRapportChantierPDF(rapport, chantier, options = {}) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    const footerLeft = `${entreprise.nom || 'Mallettico'} · Rapport ${rapport.numero || ''}`;
    ...
    if (subscription.isFree()) {
      ...
      doc.text('Mallettico Gratuit', pageWidth / 2, 150, { align: 'center', angle: 35 });
      ...
      doc.text('Document généré avec Mallettico — Plan Découverte (gratuit)', pageWidth / 2, 285, { align: 'center' });
    }
  }
