...
export default function App() {
  ...
  const loadData = async () => {
    ...
    ['chantiers', 'events', 'equipe', 'stocks'].forEach(k => {
      const v = localStorage.getItem('cp_' + k);
      if (v) { const p = JSON.parse(v); if (k === 'chantiers') setChantiers(p); if (k === 'events') setEvents(p); if (k === 'equipe') setEquipe(p); if (k === 'stocks') setStocks(p); }
    });
  };
  ...
  useEffect(() => {
    ...
    devis.filter(d => d.type === 'facture' && d.statut !== 'payee').forEach(d => notifs.push({ id: 'd'+d.id, type: 'danger', icon: '💰', msg: `Facture impayée: ${d.numero}`, page: 'devis' }));
  ...
  const searchResults = search.length > 1 ? [
    ...clients.filter(c => c.nom?.toLowerCase().includes(search.toLowerCase())).map(c => ({ type: 'client', ...c })),
    ...chantiers.filter(c => c.nom?.toLowerCase().includes(search.toLowerCase())).map(c => ({ type: 'chantier', ...c })),
  ].slice(0, 6) : [];
  ...
  if (!user) return (
    <div className="min-h-screen bg-slate-900 flex">
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-orange-500 to-orange-600 p-12 items-center">
        <div className="max-w-md text-white"><div className="text-6xl mb-6">🏗️</div><h1 className="text-4xl font-bold mb-4">ChantierPro</h1><p className="text-xl opacity-90">La solution tout-en-un pour gérer vos chantiers.</p></div>
      </div>
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-center text-slate-400 mt-6">{showSignUp ? 'Déjà inscrit ?' : 'Pas de compte ?'}<button onClick={() => setShowSignUp(!showSignUp)} className="text-orange-500 font-semibold ml-2">{showSignUp ? 'Se connecter' : "S'inscrire"}</button></p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b px-4 lg:px-6 py-3 flex items-center gap-4">
          ...
          <div className="flex-1 max-w-xl relative">
            ...
            {searchResults.length > 0 && <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border z-50">{searchResults.map((r, i) => <button key={i} onClick={() => { setPage(r.type === 'client' ? 'clients' : 'chantiers'); setSearch(''); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 text-left"><span>{r.type === 'client' ? '👤' : '🏗️'}</span><span>{r.nom}</span></button>)}</div>}
          </div>
        ...
        <main className="p-4 lg:p-6">
          {page === 'dashboard' && <Dashboard stats={stats} clients={clients} devis={devis} chantiers={chantiers} events={events} setPage={setPage} />}
          ...
          {page === 'devis' && <Devis clients={clients} devis={devis} onSubmit={async d => { await devisDB.create(d); const r = await devisDB.getAll(); if (r.data) setDevis(r.data); }} onUpdate={async (id, d) => { await devisDB.update(id, d); const r = await devisDB.getAll(); if (r.data) setDevis(r.data); }} />}
          {page === 'equipe' && <Equipe equipe={equipe} setEquipe={e => { setEquipe(e); save('equipe', e); }} />}
...
function Dashboard({ stats, clients, devis, chantiers, events, setPage }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {stats.impayees.length > 0 && <div onClick={() => setPage('devis')} className="flex items-center gap-4 p-4 bg-red-50 border border-red-200 rounded-2xl cursor-pointer"><span className="text-2xl">⚠️</span><span className="font-semibold text-red-800">{stats.impayees.length} facture(s) impayée(s)</span><span className="ml-auto">→</span></div>}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-5 text-white"><p className="text-sm opacity-80">CA du mois</p><p className="text-3xl font-bold mt-1">{stats.caMois.toLocaleString('fr-FR')} €</p></div>
        ...
        <div onClick={() => setPage('clients')} className="bg-white rounded-2xl p-5 border cursor-pointer hover:shadow-lg"><p className="text-sm text-slate-500">Clients</p><p className="text-3xl font-bold mt-1">{clients.length}</p></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border p-5">
            ...
            <div className="space-y-2">{[{icon: '📝', label: 'Créer un devis', p: 'devis'}, {icon: '👤', label: 'Ajouter client', p: 'clients'}, {icon: '🏗️', label: 'Nouveau chantier', p: 'chantiers'}].map((a, i) => <button key={i} onClick={() => setPage(a.p)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 text-left"><span>{a.icon}</span>{a.label}</button>)}</div>
          </div>
        </div>
      </div>
    </div>
...
function Chantiers({ chantiers, setChantiers, clients }) {
  ...
  const submit = () => { if (!form.nom) return; setChantiers([...chantiers, { id: Date.now().toString(), ...form, budget_prevu: parseFloat(form.budget_prevu) || 0, progression: parseInt(form.progression) || 0 }]); setShow(false); setForm({ nom: '', client_id: '', adresse: '', date_debut: '', date_fin_prevue: '', budget_prevu: '', statut: 'prospect', progression: 0 }); };
  if (show) return (
    <div className="space-y-6">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Type</label><select className="w-full px-4 py-2.5 border rounded-xl" value={form.type} onChange={e => setForm(p => ({...p, type: e.target.value}))}><option value="rdv">RDV</option><option value="chantier">Chantier</option><option value="relance">Relance</option></select></div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border p-5">
          ...
          <div className="grid grid-cols-7 gap-1">{getDays().map((d, i) => d ? <button key={i} onClick={() => setSelected(d)} className={`aspect-square rounded-xl flex items-center justify-center text-sm ${d.toISOString().split('T')[0] === selStr ? 'bg-orange-500 text-white' : 'hover:bg-slate-100'}`}>{d.getDate()}</button> : <div key={i} />)}</div>
        </div>
        <div className="bg-white rounded-2xl border">
          ...
          <div className="p-4">{selEvents.length === 0 ? <p className="text-slate-500 text-center py-8">Aucun événement</p> : <div className="space-y-3">{selEvents.map(e => <div key={e.id} className="p-3 bg-slate-50 rounded-xl"><div className="flex justify-between"><span className="text-sm font-medium">{e.time}</span><button onClick={() => setEvents(events.filter(x => x.id !== e.id))} className="text-red-500">✕</button></div><p className="font-medium">{e.title}</p></div>)}</div>}</div>
        </div>
      </div>
    </div>
  );
...
function Clients({ clients, devis, onSubmit }) {
  ...
  const submit = () => { if (!form.nom || !form.email) return; onSubmit(form); setShow(false); setForm({ nom: '', prenom: '', email: '', telephone: '', adresse: '' }); };
  if (show) return (
    <div className="space-y-6">
  ...
  return (
    <div className="space-y-6">
      ...
      {clients.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">👥</p><h3>Aucun client</h3></div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{clients.map(c => { const ca = devis.filter(d => d.client_id === c.id && d.statut === 'payee').reduce((s, d) => s + (d.total_ttc || 0), 0); return (<div key={c.id} className="bg-white rounded-2xl border p-5"><div className="flex gap-3 mb-4"><div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center text-white font-bold">{c.nom?.[0]}</div><div><h3 className="font-semibold">{c.nom} {c.prenom}</h3><p className="text-sm text-slate-500">{c.entreprise}</p></div></div><p className="text-sm text-slate-500">📧 {c.email}</p><p className="text-sm text-slate-500">📱 {c.telephone}</p><div className="flex justify-around mt-4 pt-4 border-t text-center"><div><p className="font-bold">{devis.filter(d => d.client_id === c.id).length}</p><p className="text-xs text-slate-500">Docs</p></div><div><p className="font-bold text-orange-500">{ca.toLocaleString('fr-FR')} €</p><p className="text-xs text-slate-500">CA</p></div></div></div>); })}</div>}
    </div>
  );
}

function Devis({ clients, devis, onSubmit, onUpdate }) {
  const [show, setShow] = useState(false);
  const [filter, setFilter] = useState('all');
  const [form, setForm] = useState({ clientId: '', date: new Date().toISOString().split('T')[0], type: 'devis', lignes: [] });
  const [ligne, setLigne] = useState({ description: '', quantite: 1, prixUnitaire: 0 });
  ...
  const addLigne = () => { if (!ligne.description) return; setForm(p => ({ ...p, lignes: [...p.lignes, { ...ligne, montant: ligne.quantite * ligne.prixUnitaire }] })); setLigne({ description: '', quantite: 1, prixUnitaire: 0 }); };
  const totaux = form.lignes.reduce((s, l) => s + (l.montant || 0), 0);
  const submit = () => { if (!form.clientId || form.lignes.length === 0) return alert('Client et lignes requis'); onSubmit({ client_id: form.clientId, numero: `${form.type === 'devis' ? 'DEV' : 'FACT'}-${Date.now()}`, date: form.date, type: form.type, statut: 'brouillon', lignes: form.lignes, total_ht: totaux, tva: totaux * 0.2, total_ttc: totaux * 1.2 }); setShow(false); setForm({ clientId: '', date: new Date().toISOString().split('T')[0], type: 'devis', lignes: [] }); };
  ...
  if (show) return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap"><input placeholder="Description" className="flex-1 min-w-[200px] px-3 py-2 border rounded-lg" value={ligne.description} onChange={e => setLigne(p => ({...p, description: e.target.value}))} /><input type="number" placeholder="Qté" className="w-20 px-3 py-2 border rounded-lg" value={ligne.quantite} onChange={e => setLigne(p => ({...p, quantite: parseFloat(e.target.value) || 1}))} /><input type="number" placeholder="Prix HT" className="w-28 px-3 py-2 border rounded-lg" value={ligne.prixUnitaire || ''} onChange={e => setLigne(p => ({...p, prixUnitaire: parseFloat(e.target.value) || 0}))} /><button onClick={addLigne} className="px-4 py-2 bg-orange-500 text-white rounded-lg">+</button></div>
        {form.lignes.length > 0 && <div className="border rounded-xl overflow-hidden">{form.lignes.map((l, i) => <div key={i} className="flex px-4 py-2 border-b"><span className="flex-1">{l.description}</span><span className="w-20">{l.quantite}</span><span className="w-28 text-right">{l.montant?.toFixed(2)} €</span></div>)}<div className="bg-slate-50 p-4 text-right font-bold text-orange-500">Total TTC: {(totaux * 1.2).toFixed(2)} €</div></div>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">{[['all', 'Tous'], ['devis', 'Devis'], ['factures', 'Factures'], ['impayees', 'Impayées']].map(([k, v]) => <button key={k} onClick={() => setFilter(k)} className={`px-4 py-2 rounded-xl text-sm font-medium ${filter === k ? 'bg-orange-500 text-white' : 'bg-slate-100'}`}>{v}</button>)}</div>
      {filtered.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">📄</p><h3>Aucun document</h3></div> : <div className="bg-white rounded-2xl border overflow-hidden">{filtered.map(d => { const cl = clients.find(c => c.id === d.client_id); return (<div key={d.id} className="flex items-center px-5 py-4 border-b gap-4"><span className="flex-1 font-semibold">{d.numero}</span><span className="w-32 text-slate-600 truncate">{cl?.nom || '-'}</span><span className="w-28 text-right font-bold">{(d.total_ttc || 0).toFixed(2)} €</span><select value={d.statut} onChange={e => onUpdate(d.id, { statut: e.target.value })} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${STATUTS[d.statut] || STATUTS.brouillon}`}>{Object.keys(STATUTS).map(s => <option key={s} value={s}>{s}</option>)}</select></div>); })}</div>}
    </div>
  );
}

function Equipe({ equipe, setEquipe }) {
  const [show, setShow] = useState(false);
  const [form, setForm] = useState({ nom: '', prenom: '', role: 'ouvrier', telephone: '', tauxHoraire: '' });
  const submit = () => { if (!form.nom) return; setEquipe([...equipe, { id: Date.now().toString(), ...form, tauxHoraire: parseFloat(form.tauxHoraire) || 0 }]); setShow(false); setForm({ nom: '', prenom: '', role: 'ouvrier', telephone: '', tauxHoraire: '' }); };
  if (show) return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Nom *</label><input className="w-full px-4 py-2.5 border rounded-xl" value={form.nom} onChange={e => setForm(p => ({...p, nom: e.target.value}))} /></div>
          <div><label className="block text-sm font-medium mb-1">Prénom</label><input className="w-full px-4 py-2.5 border rounded-xl" value={form.prenom} onChange={e => setForm(p => ({...p, prenom: e.target.value}))} /></div>
          <div><label className="block text-sm font-medium mb-1">Rôle</label><select className="w-full px-4 py-2.5 border rounded-xl" value={form.role} onChange={e => setForm(p => ({...p, role: e.target.value}))}><option value="ouvrier">Ouvrier</option><option value="chef">Chef</option><option value="apprenti">Apprenti</option></select></div>
        ...
        <div className="flex justify-end gap-3 mt-6 pt-6 border-t"><button onClick={() => setShow(false)} className="px-4 py-2 bg-slate-100 rounded-xl">Annuler</button><button onClick={submit} className="px-6 py-2 bg-orange-500 text-white rounded-xl">Ajouter</button></div>
      </div>
    </div>
  );
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">Équipe ({equipe.length})</h1><button onClick={() => setShow(true)} className="px-4 py-2 bg-orange-500 text-white rounded-xl">+ Nouveau</button></div>
      {equipe.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">👷</p><h3>Aucun membre</h3></div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{equipe.map(m => <div key={m.id} className="bg-white rounded-2xl border p-5"><div className="flex gap-3 mb-4"><div className="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center text-white font-bold">{m.nom?.[0]}{m.prenom?.[0]}</div><div><h3 className="font-semibold">{m.nom} {m.prenom}</h3><p className="text-sm text-slate-500 capitalize">{m.role}</p></div></div>{m.telephone && <p className="text-sm text-slate-500">📱 {m.telephone}</p>}{m.tauxHoraire > 0 && <p className="text-sm text-slate-500">💰 {m.tauxHoraire} €/h</p>}</div>)}</div>}
    </div>
  );
}
...
function Stocks({ stocks, setStocks }) {
  const [show, setShow] = useState(false);
  const [form, setForm] = useState({ nom: '', categorie: 'Autre', quantite: 0, seuil: 5, prix: 0 });
  const lowStocks = stocks.filter(s => s.quantite <= (s.seuil || 5));
  const submit = () => { if (!form.nom) return; setStocks([...stocks, { id: Date.now().toString(), ...form, quantite: parseFloat(form.quantite) || 0, seuil: parseInt(form.seuil) || 5, prix: parseFloat(form.prix) || 0 }]); setShow(false); setForm({ nom: '', categorie: 'Autre', quantite: 0, seuil: 5, prix: 0 }); };
  ...
  if (show) return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Nom *</label><input className="w-full px-4 py-2.5 border rounded-xl" value={form.nom} onChange={e => setForm(p => ({...p, nom: e.target.value}))} /></div>
          <div><label className="block text-sm font-medium mb-1">Catégorie</label><select className="w-full px-4 py-2.5 border rounded-xl" value={form.categorie} onChange={e => setForm(p => ({...p, categorie: e.target.value}))}>{['Plomberie', 'Électricité', 'Maçonnerie', 'Peinture', 'Outillage', 'Autre'].map(c => <option key={c}>{c}</option>)}</select></div>
  ...
  return (
    <div className="space-y-6">
      ...
      {lowStocks.length > 0 && <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4"><div className="flex items-center gap-3"><span className="text-2xl">⚠️</span><span className="font-semibold text-amber-800">Stocks bas ({lowStocks.length})</span></div><div className="flex flex-wrap gap-2 mt-2">{lowStocks.map(s => <span key={s.id} className="px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-sm">{s.nom}: {s.quantite}</span>)}</div></div>}
      {stocks.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">📦</p><h3>Aucun matériau</h3></div> : <div className="bg-white rounded-2xl border overflow-hidden">{stocks.map(s => <div key={s.id} className={`flex items-center px-5 py-4 border-b gap-4 ${s.quantite <= (s.seuil || 5) ? 'bg-red-50' : ''}`}><div className="flex-1"><p className="font-medium">{s.nom}</p><p className="text-xs text-slate-500">{s.categorie}</p></div><div className="flex items-center gap-2"><button onClick={() => updateQty(s.id, -1)} className="w-8 h-8 bg-slate-100 rounded-lg font-bold">-</button><span className={`w-10 text-center font-semibold ${s.quantite <= (s.seuil || 5) ? 'text-red-600' : ''}`}>{s.quantite}</span><button onClick={() => updateQty(s.id, 1)} className="w-8 h-8 bg-slate-100 rounded-lg font-bold">+</button></div><span className="w-20 text-right">{(s.prix || 0).toFixed(2)} €</span><span className="w-24 text-right font-semibold">{((s.quantite || 0) * (s.prix || 0)).toFixed(2)} €</span></div>)}<div className="px-5 py-4 bg-slate-50 flex justify-between font-semibold"><span>Total</span><span className="text-orange-500">{stocks.reduce((sum, s) => sum + (s.quantite || 0) * (s.prix || 0), 0).toFixed(2)} €</span></div></div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="border-2 rounded-2xl p-6 text-center"><h4 className="font-bold text-lg">Solo</h4><p className="text-4xl font-bold text-orange-500 my-4">29€<span className="text-sm text-slate-500">/mois</span></p><ul className="text-sm text-slate-600 space-y-2 mb-6"><li>✅ Clients illimités</li><li>✅ Devis & Factures</li><li>✅ Calendrier</li></ul><button className="w-full py-2.5 bg-orange-500 text-white rounded-xl font-medium">Choisir</button></div>
          <div className="border-2 border-green-500 rounded-2xl p-6 text-center"><h4 className="font-bold text-lg">Pro</h4><p className="text-4xl font-bold text-green-500 my-4">59€<span className="text-sm text-slate-500">/mois</span></p><ul className="text-sm text-slate-600 space-y-2 mb-6"><li>✅ Tout du Solo</li><li>✅ Multi-utilisateurs</li><li>✅ Export comptable</li></ul><button className="w-full py-2.5 bg-green-500 text-white rounded-xl font-medium">Choisir</button></div>
        </div>
      </div>
    </div>
  );
