...
// Mode démo = 100% local, zéro réseau
const isDemo = new URLSearchParams(window.location.search).get('demo') === 'true';

// Import conditionnel Supabase (seulement si pas en mode démo)
let auth, clientsDB, devisDB;
if (!isDemo) {
  try {
    const sb = await import('./supabaseClient');
    auth = sb.auth;
    clientsDB = sb.clientsDB;
    devisDB = sb.devisDB;
  } catch (e) {
    console.log('Supabase non disponible, mode local activé');
  }
}

// Fallback auth pour mode démo
const demoAuth = {
  signUp: async () => ({ error: { message: 'Mode démo' } }),
  signIn: async () => ({ error: { message: 'Mode démo' } }),
  signOut: async () => ({ error: null }),
  getCurrentUser: async () => null,
  onAuthStateChange: () => ({ data: { subscription: { unsubscribe: () => {} } } })
};

if (!auth) auth = demoAuth;
...
export default function App() {
  // User démo
  const demoUser = { 
    email: 'demo@chantierpro.test', 
    user_metadata: { nom: 'Artisan Démo', entreprise: 'Martin Rénovation' } 
  };
  ...
  useEffect(() => {
    try {
      ...
      console.log('Erreur chargement localStorage');
    }
  }, []);

  useEffect(() => { 
    try { localStorage.setItem('cp_entreprise', JSON.stringify(entreprise)); } catch (e) {} 
  }, [entreprise]);
  ...
  const loadDemoData = () => {
    ...
    const demoDevis = [
      { id: 'dv1', numero: 'DEV-2024-001', client_id: 'd1', chantier_id: 'ch1', date: '2024-01-10', type: 'devis', statut: 'accepte', tvaRate: 10, total_ht: 2850, tva: 285, total_ttc: 3135, sections: [{ id: 's1', titre: 'Travaux', lignes: [{ id: 'l1', description: 'Démolition SDB', quantite: 8, unite: 'm²', prixUnitaire: 35, montant: 280 }, { id: 'l2', description: 'Pose carrelage sol', quantite: 8, unite: 'm²', prixUnitaire: 45, montant: 360 }, { id: 'l3', description: 'Pose carrelage mural', quantite: 25, unite: 'm²', prixUnitaire: 55, montant: 1375 }, { id: 'l4', description: 'Plomberie', quantite: 15, unite: 'h', prixUnitaire: 55, montant: 825 }] }], lignes: [{ description: 'Démolition SDB', quantite: 8, unite: 'm²', prixUnitaire: 35, montant: 280 }, { description: 'Pose carrelage sol', quantite: 8, unite: 'm²', prixUnitaire: 45, montant: 360 }, { description: 'Pose carrelage mural', quantite: 25, unite: 'm²', prixUnitaire: 55, montant: 1375 }, { description: 'Plomberie', quantite: 15, unite: 'h', prixUnitaire: 55, montant: 825 }], signature: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==', signatureDate: '2024-01-12T10:30:00' },
      { id: 'dv2', numero: 'DEV-2024-002', client_id: 'd2', chantier_id: 'ch2', date: '2024-01-18', type: 'devis', statut: 'envoye', tvaRate: 10, total_ht: 1500, tva: 150, total_ttc: 1650, sections: [{ id: 's1', titre: '', lignes: [{ id: 'l1', description: 'Peinture murs salon', quantite: 40, unite: 'm²', prixUnitaire: 25, montant: 1000 }, { id: 'l2', description: 'Peinture murs chambres', quantite: 20, unite: 'm²', prixUnitaire: 25, montant: 500 }] }], lignes: [{ description: 'Peinture murs salon', quantite: 40, unite: 'm²', prixUnitaire: 25, montant: 1000 }, { description: 'Peinture murs chambres', quantite: 20, unite: 'm²', prixUnitaire: 25, montant: 500 }] },
      { id: 'dv3', numero: 'FACT-2024-001', client_id: 'd1', chantier_id: 'ch1', date: '2024-01-25', type: 'facture', statut: 'payee', tvaRate: 10, total_ht: 940.91, tva: 94.09, total_ttc: 1035, sections: [{ id: 's1', titre: '', lignes: [{ id: 'l1', description: 'Acompte 33% - DEV-2024-001', quantite: 1, unite: 'forfait', prixUnitaire: 940.91, montant: 940.91 }] }], lignes: [{ description: 'Acompte 33% - DEV-2024-001', quantite: 1, unite: 'forfait', prixUnitaire: 940.91, montant: 940.91 }] }
    ];
    ...
    const demoEvents = [
      { id: 'ev1', title: 'Chantier Dupont', date: '2024-01-15', type: 'chantier', chantierId: 'ch1', time: '08:00' },
      { id: 'ev2', title: 'RDV Bernard cuisine', date: '2024-01-22', type: 'rdv', time: '10:00' },
      { id: 'ev3', title: 'Relance devis Martin', date: '2024-01-25', type: 'relance', time: '14:00' }
    ];
    ...
    setEntreprise({
      ...
      assurance: 'AXA Décennale N°456789', 
  ...
  const loadUserData = async () => {
    try {
      if (clientsDB) {
        const c = await clientsDB.getAll();
        if (c.data) setClients(c.data);
      }
      if (devisDB) {
        const d = await devisDB.getAll();
  ...
  const handleClientSubmit = async (data) => {
    ...
    if (!isDemo && clientsDB) {
      ...
      if (r.data) { setClients(r.data); return; }
    }
  ...
  const handleDevisSubmit = async (data) => {
    ...
    if (!isDemo && devisDB) {
      ...
      if (r.data) { setDevis(r.data); return; }
    }
  ...
  const handleSignUp = async (e) => { 
    e.preventDefault(); 
    setAuthError(''); 
    const { error } = await auth.signUp(authForm.email, authForm.password, { nom: authForm.nom, entreprise: authForm.entreprise }); 
    if (error) setAuthError(error.message); 
    else { alert('Compte créé !'); setShowSignUp(false); } 
  };
  ...
  // Login (seulement si pas en mode démo)
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900' : 'bg-slate-100'}`}>
        ...
        <nav className="p-3 space-y-1">
          {nav.map(n => (
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${page === n.id ? 'text-white' : 'text-slate-400 hover:bg-slate-800'}`} 
      ...
      <div className="lg:pl-64">
        <header className={`sticky top-0 z-30 ${isDark ? 'bg-slate-800/95' : 'bg-white/95'} backdrop-blur border-b ${isDark ? 'border-slate-700' : ''} px-4 py-3 flex items-center gap-4`}>
          ...
          <div className="relative">
            <button onClick={() => setShowNotifs(!showNotifs)} className={`relative p-2.5 rounded-xl ${isDark ? 'hover:bg-slate-700 text-white' : 'hover:bg-slate-100'}`}>
            ...
            {showNotifs && (
              <div className={`absolute top-full right-0 mt-2 w-72 ${isDark ? 'bg-slate-800' : 'bg-white'} rounded-xl shadow-xl border z-50`}>
                <div className={`px-4 py-3 border-b font-semibold ${isDark ? 'text-white border-slate-700' : ''}`}>Notifications</div>
                {notifications.length === 0 ? <p className="p-4 text-center text-slate-500">✅ Aucune alerte</p> : notifications.map(n => <div key={n.id} className={`flex items-center gap-3 px-4 py-3 text-sm border-b last:border-0 ${isDark ? 'text-slate-300 border-slate-700' : ''}`}><span>{n.icon}</span><span>{n.msg}</span></div>)}
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            {showQuickAdd && (
              <div className={`absolute top-full right-0 mt-2 w-52 ${isDark ? 'bg-slate-800' : 'bg-white'} rounded-xl shadow-xl border z-50`}>
                {[{icon:'📄',label:'Nouveau devis',p:'devis'},{icon:'🧾',label:'Nouvelle facture',p:'devis'},{icon:'🏗️',label:'Nouveau chantier',p:'chantiers'},{icon:'👤',label:'Nouveau client',p:'clients'}].map((item, i) => (
                  <button key={i} onClick={() => { setPage(item.p); setShowQuickAdd(false); }} className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm border-b last:border-0 ${isDark ? 'hover:bg-slate-700 text-slate-300 border-slate-700' : 'hover:bg-slate-50'}`}>
