...
const CATEGORIES = ['Tous', 'Plomberie', 'Ã‰lectricitÃ©', 'MaÃ§onnerie', 'Carrelage', 'Peinture', 'Menuiserie', 'MatÃ©riaux', 'Autre'];
const UNITES = ['unitÃ©', 'h', 'mÂ²', 'ml', 'forfait', 'jour', 'pot', 'sac', 'rouleau', 'kg', 'mÂ³', 'palette'];
...
export default function Catalogue({ catalogue, setCatalogue, couleur }) {
  ...
  const [form, setForm] = useState({ nom: '', prix: '', prixAchat: '', unite: 'unitÃ©', categorie: 'Autre', favori: false, stock_actuel: '', stock_seuil_alerte: '' });
  ...
  const submit = () => {
    ...
    setShow(false); setEditId(null); setForm({ nom: '', prix: '', prixAchat: '', unite: 'unitÃ©', categorie: 'Autre', favori: false, stock_actuel: '', stock_seuil_alerte: '' });
  };

  const startEdit = (item) => { setForm({ nom: item.nom || '', prix: item.prix?.toString() || '', prixAchat: item.prixAchat?.toString() || '', unite: item.unite || 'unitÃ©', categorie: item.categorie || 'Autre', favori: item.favori || false, stock_actuel: item.stock_actuel?.toString() ?? '', stock_seuil_alerte: item.stock_seuil_alerte?.toString() ?? '' }); setEditId(item.id); setShow(true); };
      ...
      <div className="bg-white rounded-2xl border p-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div><label className="block text-sm font-medium mb-1">UnitÃ©</label><select className="w-full px-4 py-2.5 border rounded-xl" value={form.unite} onChange={e => setForm(p => ({...p, unite: e.target.value}))}>{UNITES.map(u => <option key={u}>{u}</option>)}</select></div>
            <div><label className="block text-sm font-medium mb-1">CatÃ©gorie</label><select className="w-full px-4 py-2.5 border rounded-xl" value={form.categorie} onChange={e => setForm(p => ({...p, categorie: e.target.value}))}>{CATEGORIES.filter(c => c !== 'Tous').map(c => <option key={c}>{c}</option>)}</select></div>
          </div>
    ...
    <div className="space-y-6">
      ...
      {favoris.length > 0 && !showStock && (
        <div className="bg-orange-50 rounded-2xl p-5">
          ...
          <div className="flex gap-2 flex-wrap">{favoris.map(item => (<div key={item.id} className="px-4 py-2 bg-white rounded-xl shadow-sm"><span className="font-medium">{item.nom}</span><span className="text-slate-500 ml-2">{item.prix}â‚¬/{item.unite}</span></div>))}</div>
        </div>
      )}
              ...
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right font-medium">{item.prix}â‚¬</td>
                    <td className="px-4 py-3 text-right text-slate-500">{item.prixAchat || 0}â‚¬</td>
