...
export default function Clients({ clients, devis, onSubmit }) {
  ...
  const getStats = (id) => ({
    docs: devis.filter(d => d.client_id === id).length,
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} className="bg-white rounded-2xl border p-5 hover:shadow-lg">
                <div className="flex gap-3 mb-4">
                  <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center text-white font-bold">{c.nom?.[0]}</div>
                  <div>
                    <h3 className="font-semibold">{c.nom} {c.prenom}</h3>
                    {c.entreprise && <p className="text-sm text-slate-500">{c.entreprise}</p>}
                  </div>
                </div>
                <p className="text-sm text-slate-500">📧 {c.email}</p>
                <div className="flex justify-around mt-4 pt-4 border-t text-center">
                  <div><p className="font-bold">{s.docs}</p><p className="text-xs text-slate-500">Docs</p></div>
                  <div><p className="font-bold text-orange-500">{s.ca.toLocaleString('fr-FR')} €</p><p className="text-xs text-slate-500">CA</p></div>
                </div>
              </div>
            );
          })}
