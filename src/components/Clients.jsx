      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border p-5">
          <div className="flex gap-3 flex-wrap mb-4">
            ...
            {client.adresse && <button onClick={() => openGPS(client.adresse)} className="flex items-center gap-2 px-4 py-2 bg-purple-500 text-white rounded-xl text-sm">🗺️ Itinéraire</button>}
          </div>
  ...
  if (show) return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border p-6">
        ...
        <div className="flex justify-end gap-3 mt-6 pt-6 border-t"><button onClick={() => { setShow(false); setEditId(null); }} className="px-4 py-2 bg-slate-100 rounded-xl">Annuler</button><button onClick={submit} className="px-6 py-2 text-white rounded-xl" style={{background: couleur}}>{editId ? 'Enregistrer' : 'Créer'}</button></div>
      </div>
    </div>
  );
          ...
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} className="bg-white rounded-2xl border p-5 hover:shadow-lg transition">
                ...
                <div className="flex gap-4 pt-3 border-t text-sm"><span>🏗️ {s.chantiers}</span><span>📄 {s.devis}</span><span className="font-bold ml-auto" style={{color: couleur}}>{s.ca.toLocaleString()}€</span></div>
              </div>
            );
          })}
        </div>
