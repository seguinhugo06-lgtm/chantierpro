...
export default function IntegrationDetailPanel({
  ...
  const statusColors = {
    connected: { bg: 'bg-emerald-100 dark:bg-emerald-900/30', text: 'text-emerald-700 dark:text-emerald-400', label: 'Connecté' },
    disconnected: { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-500', label: 'Déconnecté' },
    error: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', label: 'Erreur' },
    expired: { bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-700 dark:text-amber-400', label: 'Expiré' },
    connecting: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-400', label: 'Connexion...' },
  };
      ...
      >
        ...
        <div className={`p-5 ${bgCard} border-b ${border}`}>
          <div className="flex items-start justify-between mb-4">
            ...
            <button onClick={onClose} className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 ${textSecondary}`}>
              <X size={18} />
            </button>
          </div>
              ...
              {integration?.lastSyncError && (
                <div className={`rounded-xl p-4 border ${isDark ? 'bg-red-900/10 border-red-800/30' : 'bg-red-50 border-red-200'}`}>
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-red-600 dark:text-red-400">Dernière erreur</p>
                      <p className="text-xs text-red-500 dark:text-red-400/80 mt-1">{integration.lastSyncError}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {integration?.status === 'connected' && (
          <div className={`p-4 border-t ${border} ${bgCard}`}>
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-red-600 border border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20 font-medium text-sm transition-colors"
            >
              <Unplug size={16} />
