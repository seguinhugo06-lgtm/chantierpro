      ...
      {filtered.length === 0 ? (
        (search || catFilter !== 'Tous') ? (
          <EmptyState
            ...
            description="Modifiez vos filtres ou ajoutez un nouvel article."
            isDark={isDark}
            couleur={couleur}
            action={{ label: 'Ajouter un article', icon: Plus, onClick: () => setShow(true) }}
          />
        ) : (
          <EmptyState
            illustration="shelf"
            title="Ajoutez vos articles et prestations habituels"
            description="Centralisez vos matériaux, tarifs et gérez vos stocks en un seul endroit."
            isDark={isDark}
            couleur={couleur}
            features={[
              { icon: Star, title: 'Articles favoris', description: 'Accès rapide à vos essentiels' },
              { icon: Box, title: 'Gestion des stocks', description: 'Alertes automatiques' },
              { icon: AlertTriangle, title: 'Calcul des marges', description: 'Prix achat vs vente' }
            ]}
            action={{ label: '+ Ajouter un article', icon: Plus, onClick: () => setShow(true) }}
            secondaryAction={{ label: 'Importer depuis le Référentiel BTP', icon: Sparkles, onClick: () => setShowArticlePicker(true) }}
          />
        )
      ) : (
