  ...
  if (show) return (
    <div className="space-y-6">
      ...
      <div className={`${cardBg} rounded-2xl border p-6`}>
        ...
        <div className={`flex justify-end gap-3 mt-6 pt-6 border-t ${isDark ? 'border-slate-700' : ''}`}><button onClick={() => setShow(false)} className={`px-4 py-2 rounded-xl ${isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100'}`}>Annuler</button><button onClick={submit} className="px-6 py-2 text-white rounded-xl" style={{background: couleur}}>{editId ? 'Enregistrer' : 'Ajouter'}</button></div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowStock(!showStock)} className={`px-4 py-2 rounded-xl text-sm ${showStock ? 'text-white' : isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100'}`} style={showStock ? {background: couleur} : {}}> Stock</button>
      ...
      {alertesStock.length > 0 && (
        <div className="bg-red-50 rounded-2xl p-4 flex items-center gap-4 flex-wrap">
          <span className="text-red-600 font-medium">⚠️  Stock bas:</span>
      ...
      {favoris.length > 0 && !showStock && (
        <div className={`rounded-2xl p-5 ${isDark ? 'bg-amber-900/30 border border-amber-700' : 'bg-amber-50'}`}>
          ...
          <div className="flex gap-2 flex-wrap">{favoris.map(item => (<div key={item.id} className={`px-4 py-2 rounded-xl shadow-sm ${isDark ? 'bg-slate-700' : 'bg-white'}`}><span className={`font-medium ${textPrimary}`}>{item.nom}</span><span className={`ml-2 ${textMuted}`}>{item.prix}€/{item.unite}</span></div>))}</div>
        </div>
      )}

      <div className="flex gap-4 flex-wrap items-center">
        <input type="text" placeholder=" Rechercher..." value={search} onChange={e => setSearch(e.target.value)} className={`flex-1 max-w-xs px-4 py-2.5 border rounded-xl ${inputBg}`} />
        <div className="flex gap-2 flex-wrap">{CATEGORIES.map(cat => (<button key={cat} onClick={() => setCatFilter(cat)} className={`px-3 py-1.5 rounded-lg text-sm ${catFilter === cat ? 'text-white' : isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100'}`} style={catFilter === cat ? {background: couleur} : {}}>{cat}</button>))}</div>
      </div>

      {filtered.length === 0 ? <div className={`${cardBg} rounded-2xl border p-12 text-center`}><p className="text-5xl mb-4">📦</p><p className={textMuted}>Catalogue vide</p></div> : (
        <div className={`${cardBg} rounded-2xl border overflow-hidden`}>
          <table className="w-full text-sm">
            <thead className={`border-b ${isDark ? 'bg-slate-700' : 'bg-slate-50'}`}><tr><th className={`text-left px-4 py-3 ${textPrimary}`}>Article</th><th className={`text-right px-4 py-3 w-24 ${textPrimary}`}>Vente</th><th className={`text-right px-4 py-3 w-24 ${textPrimary}`}>Achat</th><th className={`text-right px-4 py-3 w-20 ${textPrimary}`}>Marge</th>{showStock && <th className={`text-center px-4 py-3 w-28 ${textPrimary}`}>Stock</th>}<th className="w-20"></th></tr></thead>
            <tbody>
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} className={`border-b last:border-0 ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className={`px-4 py-3 text-right ${textMuted}`}>{item.prixAchat || 0}€</td>
