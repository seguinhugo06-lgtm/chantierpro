...
export default function OnboardingChecklist({
  ...
  const [dismissed, setDismissed] = useState(() =>
    localStorage.getItem('mallettico_onboarding_completed') === 'true' ||
    localStorage.getItem('mallettico_onboarding_checklist_dismissed') === 'true'
  );

  // Track first time checklist was seen (for 14-day auto-dismiss)
  useEffect(() => {
    if (!localStorage.getItem('mallettico_onboarding_first_seen')) {
      localStorage.setItem('mallettico_onboarding_first_seen', Date.now().toString());
    }
  }, []);
  ...
  useEffect(() => {
    if (allDone && !dismissed) {
      ...
      const timer = setTimeout(() => {
        setDismissed(true);
        localStorage.setItem('mallettico_onboarding_completed', 'true');
        localStorage.setItem('mallettico_onboarding_checklist_dismissed', 'true');
      }, 4000);
  ...
  const firstSeen = parseInt(localStorage.getItem('mallettico_onboarding_first_seen') || '0');
