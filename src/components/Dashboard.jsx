...
export default function Dashboard({ stats, devis, chantiers, depenses, pointages, equipe, catalogue, setPage, setSelectedChantier, couleur, getChantierBilan, isDark, modeDiscret, entreprise }) {
  ...
  // Code couleur: Rouge <0%, Orange 0-15%, Vert >15%
  const getMargeColor = (taux) => taux < 0 ? 'text-red-500' : taux < 15 ? 'text-amber-500' : 'text-emerald-500';
  const getMargeBg = (taux) => taux < 0 ? 'bg-red-500' : taux < 15 ? 'bg-amber-500' : 'bg-emerald-500';
  ...
  const getMonthlyData = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({ label: d.toLocaleDateString('fr-FR', { month: 'short' }), ca: monthDevis.filter(dv => dv.statut === 'payee').reduce((s, dv) => s + (dv.total_ttc || 0), 0), devis: monthDevis.filter(dv => dv.type === 'devis').reduce((s, dv) => s + (dv.total_ttc || 0), 0) });
    }
    return months;
  };
  ...
  const goToChantier = (chantierId) => { if (setSelectedChantier) setSelectedChantier(chantierId); setPage('chantiers'); };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div><h1 className="text-2xl font-bold">Tableau de bord</h1><p className="text-slate-500 text-sm">{new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</p></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`${isDark ? 'bg-gradient-to-br from-slate-800 to-slate-700' : 'bg-gradient-to-br from-white to-slate-50'} rounded-2xl p-5 border shadow-sm`}>
          <div className="flex items-center justify-between mb-3"><span className="text-2xl">💰</span><span className={`w-2 h-2 rounded-full ${getMargeBg(tauxMargeGlobal)}`}></span></div>
          ...
          <p className={`text-xs mt-1 ${getMargeColor(tauxMargeGlobal)}`}>Marge: {formatMoney(totalMarge)} ({formatPct(tauxMargeGlobal)})</p>
        </div>
        ...
        <div className={`${isDark ? 'bg-gradient-to-br from-slate-800 to-slate-700' : 'bg-gradient-to-br from-white to-slate-50'} rounded-2xl p-5 border shadow-sm cursor-pointer hover:shadow-md`} onClick={() => setPage('devis')}>
          <div className="flex items-center justify-between mb-3"><span className="text-2xl">📄</span>{stats.devisAttente > 0 && <span className="text-xs px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full">{stats.devisAttente}</span>}</div>
        ...
        <div className={`${isDark ? 'bg-gradient-to-br from-slate-800 to-slate-700' : 'bg-gradient-to-br from-white to-slate-50'} rounded-2xl p-5 border shadow-sm cursor-pointer hover:shadow-md`} onClick={() => setPage('chantiers')}>
          <div className="flex items-center justify-between mb-3"><span className="text-2xl">🏗️</span>{stats.chantiersEnCours > 0 && <span className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full">{stats.chantiersEnCours}</span>}</div>
          ...
          <div className="flex gap-3 flex-wrap">
            {alertesStock.map(item => (
              <div key={item.id} className="flex items-center gap-2 px-3 py-2 bg-red-50 rounded-xl cursor-pointer hover:bg-red-100" onClick={() => setPage('catalogue')}>
                ...
                <span className="text-xs text-red-600">{item.stock_actuel}/{item.stock_seuil_alerte}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              return (
                <div key={ch.id} onClick={() => goToChantier(ch.id)} className={`${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-gradient-to-br from-slate-50 to-white hover:shadow-lg'} rounded-xl p-4 cursor-pointer transition-all border`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0"><h4 className="font-semibold truncate">{ch.nom}</h4><p className="text-xs text-slate-500">{bilan.heuresTotal}h pointées</p></div>
                    <div className={`px-2.5 py-1 rounded-lg text-sm font-bold ${getMargeColor(bilan.tauxMarge)}`}>{formatPct(bilan.tauxMarge)}</div>
                  </div>
                  <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden mb-3"><div className="h-full rounded-full" style={{width: `${progress}%`, background: couleur}}></div></div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} rounded-2xl border p-5`}>
          <div className="flex justify-between items-center mb-4"><h3 className="font-semibold">📄 Derniers documents</h3><button onClick={() => setPage('devis')} className="text-xs" style={{color: couleur}}>Voir →</button></div>
          <div className="space-y-2">
            {devis.slice(0, 4).map(d => {
              const statusIcon = { brouillon: '⚪', envoye: '🟡', accepte: '✅', payee: '💰', refuse: '❌' }[d.statut] || '📄';
              return (<div key={d.id} onClick={() => setPage('devis')} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}><span>{statusIcon}</span><div className="flex-1 min-w-0"><p className="font-medium text-sm truncate">{d.numero}</p><p className="text-xs text-slate-500">{new Date(d.date).toLocaleDateString('fr-FR')}</p></div><p className="font-bold text-sm">{formatMoney(d.total_ttc)}</p></div>);
            })}
          </div>
        </div>

        <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} rounded-2xl border p-5`}>
          <div className="flex justify-between items-center mb-4"><h3 className="font-semibold">👷 Heures récentes</h3><button onClick={() => setPage('equipe')} className="text-xs" style={{color: couleur}}>Voir →</button></div>
          <div className="space-y-2">
            {pointages.slice(0, 4).map(p => {
              const emp = equipe.find(e => e.id === p.employeId);
              const ch = chantiers.find(c => c.id === p.chantierId);
              return (<div key={p.id} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-slate-700' : 'bg-slate-50'}`}><span>{p.approuve ? '✅' : '⏳'}</span><div className="flex-1 min-w-0"><p className="font-medium text-sm truncate">{emp?.nom} {emp?.prenom}</p><p className="text-xs text-slate-500 truncate">{ch?.nom}</p></div><div className="text-right"><p className="font-bold text-sm">{p.heures}h</p></div></div>);
            })}
          </div>
        </div>
      </div>
