...
export default function Chantiers({ chantiers, addChantier, updateChantier, clients, depenses, setDepenses, pointages, setPointages, equipe, devis, ajustements, addAjustement, deleteAjustement, getChantierBilan, couleur, modeDiscret, entreprise, selectedChantier, setSelectedChantier, catalogue, deductStock }) {
  ...
  if (view) {
    ...
    const devisMO = devisLie?.lignes?.filter(l => l.categorie === 'Main d\'œuvre').reduce((s, l) => s + (l.montant || 0), 0) || devisHT * 0.3;
            ...
            <div className="bg-white rounded-xl p-4 mt-4">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  <tr className="border-b"><td className="py-2">Main d'œuvre</td><td className="text-right">{formatMoney(devisMO)}</td><td className="text-right">{formatMoney(bilan.coutMO)}</td><td className={`text-right ${bilan.coutMO <= devisMO ? 'text-emerald-500' : 'text-red-500'}`}>{bilan.coutMO <= devisMO ? '' : '+'}{formatMoney(bilan.coutMO - devisMO)}</td></tr>
        ...
        {showMODetail && (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-bold">⏱️ Détail Main d'Œuvre</h3><button onClick={() => setShowAddMO(true)} className="px-3 py-1.5 text-sm text-white rounded-lg" style={{background: couleur}}>+ Heures</button></div>
            ...
            return (
              <div key={ch.id} onClick={() => setView(ch.id)} className={`bg-white rounded-2xl border p-5 cursor-pointer hover:shadow-lg transition ${hasAlert ? 'border-red-300' : ''}`}>
                <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {hasAlert && <span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full">⚠️ Perte</span>}
