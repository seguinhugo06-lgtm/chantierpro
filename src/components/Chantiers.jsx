...
export default function Chantiers({ chantiers, addChantier, updateChantier, clients, depenses, setDepenses, pointages, setPointages, equipe, devis, ajustements, addAjustement, deleteAjustement, getChantierBilan, couleur, modeDiscret, entreprise, selectedChantier, setSelectedChantier, catalogue, deductStock }) {
  ...
  const [form, setForm] = useState({ nom: '', client_id: '', adresse: '', date_debut: '', date_fin: '', statut: 'prospect' });
  ...
  const [showEditPointage, setShowEditPointage] = useState(null);
  ...
  // Code couleur marge: Rouge <0%, Orange 0-15%, Vert >15%
  const getMargeColor = (taux) => taux < 0 ? 'text-red-500' : taux < 15 ? 'text-amber-500' : 'text-emerald-500';
  const getMargeBg = (taux) => taux < 0 ? 'bg-red-50' : taux < 15 ? 'bg-amber-50' : 'bg-emerald-50';
  ...
  const handlePhotoAdd = (e, cat = 'pendant') => {
    ...
    reader.onload = () => {
      ...
      if (ch) updateChantier(view, { photos: [...(ch.photos || []), { id: Date.now().toString(), src: reader.result, categorie: cat, date: new Date().toISOString() }] });
    };
  ...
  const addTache = () => {
    ...
    const ch = chantiers.find(c => c.id === view);
    if (ch) { updateChantier(view, { taches: [...(ch.taches || []), { id: Date.now().toString(), text: newTache, done: false }] }); setNewTache(''); }
  };
  ...
  const addDepenseToChantier = () => {
    ...
    // Déduction stock si article catalogue
    if (newDepense.catalogueId && deductStock) deductStock(newDepense.catalogueId, 1);
    setNewDepense({ description: '', montant: '', categorie: 'Matériaux', catalogueId: '' });
  };

  // Ajustement REVENU ou DEPENSE
  const handleAddAjustement = () => {
    if (!adjForm.libelle || !adjForm.montant_ht) return;
    addAjustement({ chantierId: view, type: showAjustement, libelle: adjForm.libelle, montant_ht: parseFloat(adjForm.montant_ht) });
    setAdjForm({ libelle: '', montant_ht: '' }); setShowAjustement(null);
  };

  // Ajout MO manuel
  const handleAddMO = () => {
    if (!moForm.employeId || !moForm.heures) return;
    setPointages([...pointages, { id: Date.now().toString(), employeId: moForm.employeId, chantierId: view, date: moForm.date, heures: parseFloat(moForm.heures), note: moForm.note, manuel: true, approuve: true }]);
    setMoForm({ employeId: '', date: new Date().toISOString().split('T')[0], heures: '', note: '' }); setShowAddMO(false);
  };

  // Edition pointage
  const handleEditPointage = (id, field, value) => {
  ...
  const submit = () => {
    ...
    addChantier(form);
    setShow(false); setForm({ nom: '', client_id: '', adresse: '', date_debut: '', date_fin: '', statut: 'prospect' });
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1"><h1 className="text-2xl font-bold">{ch.nom}</h1><p className="text-slate-500">{client?.nom} • {ch.adresse}</p></div>
        ...
        <div className={`${getMargeBg(bilan.tauxMarge)} rounded-2xl border p-5`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            {/* Matériaux - Cliquable */}
            <div className="bg-white rounded-xl p-4 cursor-pointer hover:shadow-md" onClick={() => setShowMatDetail(true)}>
              <div className="flex justify-between mb-1"><p className="text-xs text-slate-500">Matériaux</p><span className="text-xs text-slate-400">Détail →</span></div>
            ...
            {/* Ajustements Dépenses - Cliquable */}
            <div className="bg-white rounded-xl p-4 cursor-pointer hover:shadow-md" onClick={() => setShowAjustement('DEPENSE')}>
              <div className="flex justify-between mb-1"><p className="text-xs text-slate-500">Autres dépenses</p><span className="text-xs px-2 py-0.5 rounded bg-red-100 text-red-700">+ Ajuster</span></div>
              <p className="text-xl font-bold text-red-500">{formatMoney(bilan.adjDepenses + bilan.fraisFixes)}</p>
            </div>
          </div>

          {/* Budget vs Réel */}
          {bilan.margePrevue > 0 && !modeDiscret && (
            <div className="bg-white rounded-xl p-4">
              <div className="flex justify-between items-center">
                <div><p className="text-xs text-slate-500">Marge prévue (devis)</p><p className="font-bold text-lg">{formatMoney(bilan.margePrevue)}</p></div>
                <div className="text-center"><p className="text-xs text-slate-500">Écart</p><p className={`font-bold text-lg ${bilan.marge >= bilan.margePrevue ? 'text-emerald-500' : bilan.marge < 0 ? 'text-red-500' : 'text-amber-500'}`}>{bilan.marge >= bilan.margePrevue ? '↑' : '↓'} {formatMoney(Math.abs(bilan.marge - bilan.margePrevue))}</p></div>
                <div className="text-right"><p className="text-xs text-slate-500">Marge actuelle</p><p className={`font-bold text-lg ${getMargeColor(bilan.tauxMarge)}`}>{formatMoney(bilan.marge)}</p></div>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4">
                ...
                {adjRevenus.map(a => (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b last:border-0">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-emerald-600">+{formatMoney(a.montant_ht)}</span>
                      <button onClick={() => deleteAjustement(a.id)} className="text-red-400 hover:text-red-600">✕</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-2xl border p-5">
                ...
                {adjDepenses.map(a => (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b last:border-0">
                    <span>{a.libelle}</span>
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-red-600">-{formatMoney(a.montant_ht)}</span>
                      <button onClick={() => deleteAjustement(a.id)} className="text-red-400 hover:text-red-600">✕</button>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-2xl border p-5">
              ...
              <div className="flex gap-2 flex-wrap">
                <select value={newDepense.catalogueId} onChange={e => { const item = catalogue?.find(c => c.id === e.target.value); if (item) setNewDepense(p => ({...p, catalogueId: e.target.value, description: item.nom, montant: item.prixAchat?.toString() || '' })); }} className="px-3 py-2.5 border rounded-xl text-sm">
                  <option value="">Catalogue...</option>
                  {catalogue?.map(c => <option key={c.id} value={c.id}>{c.nom} ({c.prixAchat}€)</option>)}
                </select>
        ...
        {activeTab === 'taches' && (
          <div className="bg-white rounded-2xl border p-5">
            ...
            {tasksTotal > 0 && <div className="w-full h-2 bg-slate-100 rounded-full mb-4 overflow-hidden"><div className="h-full rounded-full" style={{width: `${(tasksDone/tasksTotal)*100}%`, background: couleur}}></div></div>}
            <div className="space-y-2 mb-4">
              {ch.taches?.map(t => (
              ...
              <div className="flex gap-2 flex-wrap">
                {PHOTO_CATS.map(cat => (
                  <label key={cat} className="px-3 py-1.5 text-white rounded-lg cursor-pointer text-xs" style={{background: cat === 'litige' ? '#ef4444' : cat === 'avant' ? '#3b82f6' : cat === 'après' ? '#22c55e' : couleur}}>
                    + {cat}<input type="file" accept="image/*" capture="environment" onChange={e => handlePhotoAdd(e, cat)} className="hidden" />
                  </label>
                ))}
              </div>
            </div>
            {(!ch.photos || ch.photos.length === 0) ? <p className="text-slate-400 text-center py-8">Aucune photo</p> : (
              <div className="space-y-4">
                {PHOTO_CATS.map(cat => {
                  ...
                  return (<div key={cat}><p className="text-sm font-medium mb-2 capitalize">{cat} ({catPhotos.length})</p><div className="flex gap-2 flex-wrap">{catPhotos.map(p => (<div key={p.id} className="relative group"><img src={p.src} className="w-24 h-24 object-cover rounded-xl" alt="" /><button onClick={() => deletePhoto(p.id)} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full text-xs opacity-0 group-hover:opacity-100">✕</button></div>))}</div></div>);
                })}
              </div>
            )}
          </div>
        ...
        {showAjustement && (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md">
              ...
              <p className="text-sm text-slate-500 mb-4">{showAjustement === 'REVENU' ? 'Ex: Travaux supplémentaires acceptés oralement' : 'Ex: Achat imprévu, sous-traitance...'}</p>
              <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => { setShowAjustement(null); setAdjForm({ libelle: '', montant_ht: '' }); }} className="px-4 py-2 bg-slate-100 rounded-xl">Annuler</button>
                <button onClick={handleAddAjustement} className="px-4 py-2 text-white rounded-xl" style={{background: showAjustement === 'REVENU' ? '#22c55e' : '#ef4444'}}>Ajouter</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showMODetail && (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold">⏱️ Détail Main d'Œuvre</h3>
                <button onClick={() => setShowAddMO(true)} className="px-3 py-1.5 text-sm text-white rounded-lg" style={{background: couleur}}>+ Heures</button>
              </div>
              <div className="space-y-2 mb-4">
                {chPointages.map(p => {
                  const emp = equipe.find(e => e.id === p.employeId);
                  const cout = emp?.coutHoraireCharge || 28;
                  return (
                    <div key={p.id} className={`p-3 rounded-xl ${p.manuel ? 'bg-blue-50' : 'bg-slate-50'} ${p.verrouille ? 'opacity-60' : ''}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          {p.verrouille && <span className="text-xs bg-slate-400 text-white px-2 py-0.5 rounded">🔒</span>}
                        </div>
                        {!p.verrouille && <button onClick={() => deletePointage(p.id)} className="text-red-400">🗑️</button>}
                      </div>
                      <div className="grid grid-cols-4 gap-2 mt-2 text-sm">
                        <div><p className="text-xs text-slate-500">Date</p><input type="date" value={p.date} onChange={e => handleEditPointage(p.id, 'date', e.target.value)} disabled={p.verrouille} className="w-full px-2 py-1 border rounded text-xs" /></div>
                        <div><p className="text-xs text-slate-500">Employé</p><p className="font-medium">{emp?.nom}</p></div>
                        <div><p className="text-xs text-slate-500">Heures</p><input type="number" step="0.5" value={p.heures} onChange={e => handleEditPointage(p.id, 'heures', e.target.value)} disabled={p.verrouille} className="w-full px-2 py-1 border rounded" /></div>
                        <div><p className="text-xs text-slate-500">Coût</p><p className="font-bold text-blue-600">{formatMoney(p.heures * cout)}</p></div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="border-t pt-4 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-blue-600">{formatMoney(bilan.coutMO)}</span>
              </div>
            ...
            <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="date" className="px-4 py-2.5 border rounded-xl" value={moForm.date} onChange={e => setMoForm(p => ({...p, date: e.target.value}))} />
                  <input type="number" step="0.5" placeholder="Heures *" className="px-4 py-2.5 border rounded-xl" value={moForm.heures} onChange={e => setMoForm(p => ({...p, heures: e.target.value}))} />
                </div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setShowAddMO(false)} className="px-4 py-2 bg-slate-100 rounded-xl">Annuler</button>
        ...
        {showMatDetail && (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-2 mb-4">
                {chDepenses.map(d => (
                  <div key={d.id} className="flex justify-between p-3 bg-slate-50 rounded-xl">
                    <div><p className="font-medium">{d.description}</p><p className="text-xs text-slate-500">{new Date(d.date).toLocaleDateString('fr-FR')} • {d.categorie}</p></div>
                    <span className="font-bold text-red-500">{formatMoney(d.montant)}</span>
                  </div>
                ))}
                {chDepenses.length === 0 && <p className="text-center text-slate-400 py-4">Aucune dépense</p>}
              ...
              <button onClick={() => setShowMatDetail(false)} className="w-full mt-4 py-2 bg-slate-100 rounded-xl">Fermer</button>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">Chantiers ({chantiers.length})</h1><button onClick={() => setShow(true)} className="px-4 py-2 text-white rounded-xl" style={{background: couleur}}>+ Nouveau</button></div>
      {chantiers.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">🏗️</p><p className="text-slate-500">Aucun chantier</p></div> : (
        <div className="grid gap-4">
          {chantiers.map(ch => {
            ...
            return (
              <div key={ch.id} onClick={() => setView(ch.id)} className="bg-white rounded-2xl border p-5 cursor-pointer hover:shadow-lg transition">
                <div className="flex justify-between items-start mb-3">
                  <div><h3 className="font-semibold text-lg">{ch.nom}</h3><p className="text-sm text-slate-500">{client?.nom} • {ch.adresse}</p></div>
