      ...
      <div className="bg-white rounded-2xl border p-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium mb-1">Coût horaire chargé (â‚¬/h) *</label>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b pb-2 overflow-x-auto">
        {[['pointage', 'â±ï¸ Pointage'], ['equipe', 'ðŸ‘· Équipe'], ['validation', `âœ… Validation ${pointagesEnAttente.length > 0 ? `(${pointagesEnAttente.length})` : ''}`], ['historique', 'ðŸ“‹ Historique']].map(([k, v]) => (
      ...
      {tab === 'equipe' && (
        equipe.length === 0 ? <div className="bg-white rounded-2xl border p-12 text-center"><p className="text-5xl mb-4">ðŸ‘·</p><p className="text-slate-500">Ajoutez votre équipe</p></div> : (
            ...
            {equipe.map(e => (
              <div key={e.id} className="bg-white rounded-2xl border p-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                  <div className="bg-slate-50 p-3 rounded-xl"><p className="text-xs text-slate-500">Taux facturé</p><p className="font-bold">{e.tauxHoraire || 45}â‚¬/h</p></div>
                  <div className="bg-red-50 p-3 rounded-xl"><p className="text-xs text-slate-500">Coût chargé</p><p className="font-bold text-red-600">{e.coutHoraireCharge || 28}â‚¬/h</p></div>
                </div>
